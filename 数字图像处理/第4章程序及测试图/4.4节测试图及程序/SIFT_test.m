...
signal = interp2( signal, X, Y, '*linear' );  %内插值？？？？？？？？？？？？？？ 
subsample = [0.5]; %  降采样率；
...
if interactive >= 2
   ...
   % clf;
   imshow(pic);
   title('The gaussian pyramid (0.25 scale)')
...
if interactive >= 2
   ...
   clf;
   imagesc(pic);
   title('The DOG pyramid (0.25 scale)');
   resizeImageFig( fig, size(pic), 0.25 );
...
%  clear raw_keypoints contrast_keypoints curve_keypoints
  raw_keypoints; contrast_keypoints; curve_keypoints;

% 下一步是计算特征点的主方向.  
% 在特征点的一个区域内计算其梯度直方图
g = gaussian_filter( 1.5 * absolute_sigma(1,intervals+3) / subsample(1) );
zero_pad = ceil( length(g) / 2 );   %  B = ceil(A) rounds theelements of A to the nearest integers greater thanor equal to A
...
%    clear mag grad
mag;mag_pyr;grad;grad_pyr; %++++++I added them+++++++++++++++++++++++
grad_time = toc;
if interactive >= 1
